...
class UaqsApi extends ApiCalls {
  UaqsApi(Duration timeout) : super(timeout);

  Future<ApiResponse> getUnansweredQuestions({int counter = 0}) async {
    ...
    try {
      response = await http.get(
          Uri.parse(baseGatewayUrl + 'feed/getUnAnsweredQuestion'),
          headers: headers);
    } catch (e) {
      return ApiResponse(
          success: false,
...
class SearchApi extends ApiCalls {
  SearchApi(Duration timeout) : super(timeout);

  Future<ApiResponse> search({String username, int counter = 0}) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    ...
    try {
      response = await http.get(
          Uri.parse(baseGatewayUrl + 'search/find/$username'),
          headers: headers);
    } catch (e) {
      return ApiResponse(
          success: false,
